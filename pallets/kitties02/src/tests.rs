...
fn create_kitty_failed() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Kitties02Module::create(Origin::signed(1)), Error::<Test>::InvalidKittyId);
	})
}
...
fn breed_kitty_works() {
	new_test_ext().execute_with(|| {
		...
		// 繁殖一个新kitty, 对应账户1
		assert_ok!(Kitties02Module::breed(Origin::signed(1), 0, 1));

		// 查询kitty数量，预期为3个
		assert_eq!(NextKittyId::<Test>::get(), 3);

		// 经过繁殖，预期账户1下有3个kitty
		assert_eq!(AllKitties::<Test>::take(1).len(), 3);
	})
}
...
fn breed_kitty_failed_same_kitty_id() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Kitties02Module::breed(Origin::signed(1), 0, 0), Error::<Test>::SameKittyId);
	})
}
...
fn breed_kitty_failed_invalid_parent_kitty_id() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(Kitties02Module::breed(Origin::signed(1), 0, 1), Error::<Test>::InvalidKittyId);
	})
}
...
fn transfer_kitty_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Kitties02Module::transfer(Origin::signed(1), 0, 2));
...
fn transfer_kitty_failed_kitty_id_not_exists() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Kitties02Module::transfer(Origin::signed(1), 1, 2),
...
fn transfer_kitty_failed_not_owner() {
	new_test_ext().execute_with(|| {
		...
		// 预期将报错
		assert_noop!(Kitties02Module::transfer(Origin::signed(1), 1, 1), Error::<Test>::NotOwner);
	})
}
